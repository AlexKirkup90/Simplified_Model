...
def test_clean_extreme_moves(monkeypatch):
    ...
    cleaned = backend.clean_extreme_moves(df, max_daily_move=0.30, min_price=1.0)
...
def test_fill_missing_data(monkeypatch):
    ...
    filled = backend.fill_missing_data(df, max_gap_days=3)

    expected = pd.DataFrame({"A": [1.0, 1.0, 3.0]}, index=idx)
    pd.testing.assert_frame_equal(filled, expected)
    assert messages == ["🔧 Data filling: Filled 3 missing data points with interpolation"]
...
def test_validate_and_clean_market_data(monkeypatch):
    ...
    cleaned, alerts = backend.validate_and_clean_market_data(df)
    ...
    assert messages == [
        "🧹 Data cleaning: Fixed 2 extreme price moves across all stocks",
        "🔧 Data filling: Filled 5 missing data points with interpolation",
    ]
