...
def get_gist_config():
    try:
        ...
        return GIST_API_URL, HEADERS, FILENAME
    except Exception as e:
        st.sidebar.error("🔒 Secrets not found or invalid.")
        raise e
...
def generate_live_portfolio(momentum_window: int, top_n: int, cap: float) -> tuple[pd.DataFrame | None, pd.DataFrame | None]:
    ...
    if top_performers.empty:
        st.warning("⚠️ No stocks with positive momentum found. Recommending cash.")
        return pd.DataFrame(), pd.DataFrame()

    st.info("⚖️ Calculating portfolio weights...")
...
def save_portfolio_to_gist(portfolio_df: pd.DataFrame):
    ...
    try:
        GIST_API_URL, HEADERS, FILENAME = get_gist_config()
        ...
        st.sidebar.error(f"❌ Unexpected error during save: {e}")
