    ...
    try:
        ...
        if isinstance(df.columns, pd.MultiIndex):
            ...
            if "Close" in df.columns and "Volume" in df.columns:
                close = df[["Close"]].copy()
                vol = df[["Volume"]].copy()
                # If a single ticker was requested, set column name to that ticker
                if len(tickers) == 1:
...
def generate_live_portfolio_classic(
    momentum_window: int,
    top_n: int,
    cap: float,
    ...
    if min_dollar_volume > 0:
        ...
        keep = filter_by_liquidity(close[available], vol[available], min_dollar_volume) if available else []
        if not keep:
            return None, None
        close = close[keep]
...
def generate_live_portfolio_isa(
    preset: Dict,
...
# =========================
# Backtests for app (with costs & liquidity) — includes available tickers fix
# =========================
